...
| T-004 | Install Alpaca CLI; verify auth; **validate a multi-leg (`mleg`) order via `--dry-run`** on the dev account | A | **P0** | ⚠️ **Still not done against a live account** (no Alpaca keys available while this codebase was built — see `.env.example`). Code path exists (`agent/execution.py` uses `alpaca-py`'s typed `OptionLegRequest`/`OrderClass.MLEG`, not the raw CLI escape hatch — see [research.md](research.md) §1.1 for why the SDK path was chosen over the undocumented CLI flags) but is **untested against the live API**. First thing to run once a paper account exists: `python -m agent.daemon --once`. |
| T-005 | Verify option chain + snapshot greeks are returned on the free indicative feed for our candidate tickers | A | **P0** | Same caveat as T-004 — code assumes this works (per [research.md](research.md) §1.4) but is unverified live |
...
| T-021 | **Go-live smoke test on the competition account** | A + T | **P0** | ⏳ **Blocked on T-001** (competition account not created yet) — this is now genuinely the next step once accounts exist |
